...
echo ============================================================
echo [RUFF] AstrBot Plugin Code Formatting Script
echo ============================================================
...
echo [STEP 1/7] Entering AstrBot root directory...
cd /d "%ASTRBOT_ROOT%"
if errorlevel 1 goto :err_root

echo [STEP 2/7] Checking virtual environment activation script...
if not exist "%PLUGIN_DIR%\.venv\Scripts\activate.bat" goto :err_venv

echo [STEP 3/7] Activating virtual environment...
call "%PLUGIN_DIR%\.venv\Scripts\activate.bat"
if errorlevel 1 goto :err_activate

echo [STEP 4/7] Showing Ruff version info...
ruff --version
if errorlevel 1 goto :err_ruff

echo [STEP 5/7] Returning to plugin directory...
cd /d "%PLUGIN_DIR%"
...
REM Parse ruff format output (supports file/files and ANSI colors)
...
echo.
echo [STEP 6/7] Collecting issue count before fixing (JSON)...
...
echo.
echo [STEP 7/7] Collecting remaining issue count after fixing (JSON)...
...
:err_root
echo [ERROR] Cannot enter AstrBot root directory: %ASTRBOT_ROOT%
goto :fail

:err_venv
echo [ERROR] Virtual environment activation script not found: %PLUGIN_DIR%\.venv\Scripts\activate.bat
goto :fail
...
:err_ruff
echo [ERROR] Cannot run ruff --version. Please make sure ruff is installed in the current virtual environment
goto :fail

:err_plugin
echo [ERROR] Cannot return to plugin directory: %PLUGIN_DIR%
goto :fail
...
:err_remaining
echo [ERROR] There are still unfixed issues. Please fix them based on the output above and retry.
goto :fail
...
:fail
echo.
echo [FAILED] Script execution failed. Please troubleshoot based on the log above.
set "EXIT_CODE=1"
